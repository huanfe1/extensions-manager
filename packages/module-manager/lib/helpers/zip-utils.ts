...
export const zipAndDeleteModule = async (moduleName: ModuleNameType, pagesPath: string, archivePath: string, testsPath?: string) => {
...
export const zipFolder = async (folderPath: string, out: string, filesToInclude?: string[]) => {
    const fileList = await fg(['!node_modules', '!dist', '!**/*/node_modules', '!**/*/dist'].concat(filesToInclude?.length ? filesToInclude : ['**/*']), {
        cwd: folderPath,
        onlyFiles: true,
    });
