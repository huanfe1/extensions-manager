...
import type { ChoicesType, CliEntriesType, InputConfigType, ModuleNameType, WritableModuleConfigValuesType } from '../types.js';
...
export const processModuleConfig = (manifestObject: ManifestType, moduleName: ModuleNameType, isRecovering?: boolean) => {
    ...
    if (moduleName === 'content' || moduleName === 'content-ui') {
        if (isRecovering) {
            (moduleConfigValues as WritableModuleConfigValuesType<typeof moduleName>).content_scripts.map(script => manifestObject.content_scripts?.push(script));
        } else {
            const outputFileName = new RegExp(`${moduleName}/+`);

            manifestObject.content_scripts = manifestObject.content_scripts?.filter(script => !outputFileName.test(script.js ? script.js[0] : ''));
        }
        return;
    }
    ...
    moduleConfigEntriesOfKeys.forEach(([key, value]) => {
        ...
        if (manifestValue) {
            if (manifestValue instanceof Array) {
                const arrayValues = Object.values(moduleConfigValues[key as ConditionalPickDeep<keyof typeof moduleConfigValues, typeof moduleName>]);
...
export const processSelection = async (choices: ChoicesType, question: typeof RECOVER_CHOICE_QUESTION | typeof DELETE_CHOICE_QUESTION, moduleName?: ModuleNameType) => {
    if (!choices.length) {
