...
import { BUILD_COMPLETE, DONE_UPDATE, DO_UPDATE, LOCAL_RELOAD_SOCKET_PORT, LOCAL_RELOAD_SOCKET_URL } from '../consts.js';
    ...
    wss.on('connection', ws => {
        ...
        ws.addEventListener('message', event => {
            ...
            if (message.type === BUILD_COMPLETE) {
                clientsThatNeedToUpdate.forEach((ws: WebSocket) => ws.send(MessageInterpreter.send({ type: DO_UPDATE, id: message.id })));
            }
        });
    });
