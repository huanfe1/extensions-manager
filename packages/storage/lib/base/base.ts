...
const updateCache = async <D>(valueOrUpdate: ValueOrUpdateType<D>, cache: D | null): Promise<D> => {
    ...
    const isFunction = <D>(value: ValueOrUpdateType<D>): value is (prev: D) => D | Promise<D> => typeof value === 'function';
 ...
 */
export const createStorage = <D = string>(key: string, fallback: D, config?: StorageConfigType<D>): BaseStorageType<D> => {
    ...
    if (globalSessionAccessLevelFlag === false && storageEnum === StorageEnum.Session && config?.sessionAccessForContentScripts === true) {
